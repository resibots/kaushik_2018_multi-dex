...
/** Additional models */ 
#include <multidex/model/random_forest/random_forest.hpp>
...
struct Params{    
    struct multidex{
       BO_PARAM(size_t, action_dim, 2); //2 velocity controls
        BO_PARAM(size_t, model_input_dim, 3); //2 positions + 1 button
        BO_PARAM(size_t, model_pred_dim, 3); // 2 positions + 1 button
        ...
        BO_PARAM_ARRAY(double, limits, 1.82, M_PI / 2.0, 1.); //TODO: model input absolute max
        BO_PARAM_ARRAY(double, limits_low, -1.82, - M_PI / 2.0, 0.); //Model input low
        BO_PARAM_ARRAY(double, model_pred_limits, 1.82, M_PI / 2.0, 1.); //NOTE:: For behavior normalization and constraining pred
        BO_PARAM_ARRAY(double, model_pred_limits_low, -1.82, - M_PI / 2.0, 0.); //NOTE:: For behavior normalization and constraining pred
        ...
        BO_PARAM(double, behavior_buffer_size, 20); //Only keep N most novel behaviors for novelty computation
    }; 
...
/** Reward model Parameters */
    struct model_randomForest : public ::multidex::defaults::model_randomForest{
        BO_PARAM(int, max_depth, 20);
        BO_PARAM(double, regression_accuracy, 0.0001);
...
struct policyParams {
    struct nn_policy {
        ...
        BO_PARAM_ARRAY(double, max_u, 2.0, 2.0); //NOTE Policy output limits
        BO_PARAM_ARRAY(double, limits, 1.82, M_PI / 2.0, 1); //NOTE: Policy input limits for normalization        
...
namespace global {
    ...
    //Reward model to be trained after every execution
    using rewardModel_t = multidex::model::RandomForest<Params::model_randomForest>;
    rewardModel_t rewardModel(Params::multidex::model_input_dim(),1);
}

/** The reward function to be used by the robot during execution**/
struct ActualRewardFunction{  
    double operator()(const Eigen::VectorXd& from_state, const Eigen::VectorXd& action, const Eigen::VectorXd& to_state) const
    {
        Eigen::VectorXd actuator_state = to_state.head(2);
        using robot_simu_t = robot_dart::RobotDARTSimu<robot_dart::robot_control<robot_dart::PositionControl>>;
        static int count = 0, episode = 0;
        ...
        for (int i = 0; i < actuator_state.size(); i++){
            params[i] = actuator_state(i);
        }
        robot_simu_t simu(params, simulated_robot);

        simulated_robot->skeleton()->setPositions(actuator_state.head(2));
        simu.run(2.*simu.step());

        auto bd = simulated_robot->skeleton()->getBodyNode("end_eff");
        Eigen::VectorXd eef = bd->getTransform().translation();

        if((eef - global::button).norm() < 0.1) //Button radius is 0.1
        {
            global::switch_on = true;
        }
    
        if((eef - global::goal).norm() < 0.1 && global::switch_on) //Goal radius is 0.1
        {
            return std::exp(-(0.5/(0.2*0.2))*(eef - global::goal).squaredNorm());
        }
        return 0;
    }
};

/** The reward function to be used by the multi-dex for optimization of policy**/
struct LearnedRewardFunction{
    double operator()(const Eigen::VectorXd& from_state, const Eigen::VectorXd& action, const Eigen::VectorXd& to_state) const
    {
        return global::rewardModel.mu(to_state)[0];
    }
};
...
void init_simu(const std::string& robot_file)
{
    ...
    global::goal[1] = 0; 
    global::goal[2] =  0.6;
    global::button = global::goal;
    global::button(0) = - global::goal(0);

    std::shared_ptr<robot_dart::Robot> simulated_robot = global::global_robot->clone();
    simulated_robot->fix_to_world();
    std::cout << "Goal position: " << global::goal.transpose() <<std::endl;
}

Eigen::VectorXd get_robot_state(const std::shared_ptr<robot_dart::Robot>& robot)
{
    Eigen::VectorXd pos = robot->skeleton()->getPositions();
    Eigen::VectorXd state(Params::multidex::model_input_dim());
    state.head(2) = pos;
    ...
    robot_simu_t simu(params, simulated_robot);
    simulated_robot->skeleton()->setPositions(pos);
    simu.run(2.*simu.step());
    auto bd = simulated_robot->skeleton()->getBodyNode("end_eff");
    Eigen::VectorXd eef = bd->getTransform().translation();

    if((eef - global::button).norm() < 0.1)
    {
        global::switch_on = true;
    }  

    if(global::switch_on == true)
    {
...
struct Manipulator : public multidex::system::DARTSystem<Params, PolicyBasedController> 
{
    ...
    {
        Eigen::VectorXd transformed = original_state;
        if (original_state(2) > 0.3)
            transformed(2) = 1.;
        else
            transformed(2) = 0.;
        return transformed;
    ...
    void add_extra_to_simu(base_t::robot_simu_t& simu) const
    {
        ...
        simu.add_ellipsoid(goal_pose, {0.1*2, 0.1*2, 0.1*2}, "fixed", 1., dart::Color::Green(0.5), "goal_marker_boundary");
        simu.world()->getSkeleton("goal_marker_boundary")->getRootBodyNode()->setCollidable(false);

        Eigen::Vector6d button_pose = Eigen::Vector6d::Zero();
        button_pose.tail(3) = global::button;
        simu.add_ellipsoid(button_pose, {0.1*2, 0.1*2, 0.1*2}, "fixed", 1., dart::Color::Blue(0.3), "switch_marker");
        simu.world()->getSkeleton("switch_marker")->getRootBodyNode()->setCollidable(false);
    ...
    obsDynamics_t execute(const Policy& policy, const Reward& world, double T, std::vector<double>& R, bool& collision)
    {
        ...
        global::switch_on = false; //NOTE
        assert(Params::dart_system::sim_step() < Params::multidex::dt());
        obsDynamics_t res;

        ActualRewardFunction rew_func;
        ...
        {
            Eigen::VectorXd init_raw = states[j];
            Eigen::VectorXd init = this->transform_state(init_raw);
            Eigen::VectorXd u = commands[j];
            Eigen::VectorXd final = states[j + 1];
        
            double r = rew_func(init_raw, u, final);
            R.push_back(r);
        
            res.push_back(std::make_tuple(init, u, final - init_raw, init_raw));
        }
...
// -------------------------------------------------
using rewardFunction_t = LearnedRewardFunction;
...
multidex::Multidex<Params, global::policy_t, rewardFunction_t, Manipulator, Dynamics_model_t> mdx;
// -------------------------------------------------
...
int main(int argc, char** argv)
{
    ...
    if (env_p)
    {
        init_simu(std::string(RESPATH) + "/URDF/omnigrasper_squarePeg_2d.urdf"); //if it does not exist, we might be running this on the cluster
    }
    else
    {
        init_simu("/nfs/hal01/rkaushik/shared/URDF/omnigrasper_squarePeg_2d.urdf");
    }
    ...
    while(total_episode_count < 2)
    {
        ...
        /*LEARN REWARD MODEL*/
        std::vector<Eigen::VectorXd> obs_states, obs_rewards;     
        for (size_t i = 0; i < mdx._all_step_rewards.size(); i++) {
            obs_rewards.push_back(limbo::tools::make_vector(mdx._all_step_rewards[i]));
            obs_states.push_back(std::get<3>(mdx._obs_dynamics[i]) + std::get<2>(mdx._obs_dynamics[i]));                      
        }
        global::rewardModel.compute(obs_states, obs_rewards);
        std::cout<<"Learned new reward model with "<<obs_states.size()<<" data points"<<std::endl;
